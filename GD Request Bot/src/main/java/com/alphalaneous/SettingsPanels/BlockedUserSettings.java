...
public class BlockedUserSettings {
    ...
    private static final Path file = Paths.get(Defaults.saveDirectory + "\\GDBoard\\blockedUsers.txt");
    ...
    public static void removeBlockedUser(String ID){
        if (Files.exists(file)) {
            try {
                Path temp = Paths.get(Defaults.saveDirectory + "\\GDBoard\\_tempUsers_");
