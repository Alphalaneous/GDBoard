...
public class BlockedIDSettings {

    private static final FancyTextArea blockedInput = new FancyTextArea(true, false);
    private static final Path file = Paths.get(Defaults.saveDirectory + "\\GDBoard\\blocked.txt");
    ...
    public static void removeBlockedLevel(String ID){
        if (Files.exists(file)) {
            try {
                Path temp = Paths.get(Defaults.saveDirectory + "\\GDBoard\\_tempIDs_");
