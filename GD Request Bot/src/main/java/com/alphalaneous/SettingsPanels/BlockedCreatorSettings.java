...
public class BlockedCreatorSettings {
    ...
    private static final Path file = Paths.get(Defaults.saveDirectory + "\\GDBoard\\blockedGDUsers.txt");
    private static final ListView listView = new ListView("$BLOCKED_CREATORS$");
    ...
    public static JPanel createPanel(){
        ...
        listView.addInput("\uF0D1", blockedInput, () -> {
            addBlockedCreator(listView.getInputText());
    ...
    public static void removeBlockedCreator(String ID){
        if (Files.exists(file)) {
            try {
                Path temp = Paths.get(Defaults.saveDirectory + "\\GDBoard\\_tempCreators_");
    ...
    public static void addBlockedCreator(String ID){
        try {
            if (!Files.exists(file)) {
    ...
    public static CurvedButton createButton(String text){
        ...
        button.addActionListener(e -> new Thread(() ->{
            String option = DialogBox.showDialogBox("$UNBLOCK_CREATOR_DIALOG_TITLE$", "$UNBLOCK_CREATOR_DIALOG_INFO$", "", new String[]{"$YES$", "$NO$"}, new Object[]{button.getLText()});
