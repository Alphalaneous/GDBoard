...
public class LevelButton extends CurvedButtonAlt {
	...
	public void select(boolean refresh) {
		...
		if (Requests.getPosFromID(ID) != -1) {
			JLabel likes = createLabel("Likes: " + RequestsTab.getRequest(Requests.getPosFromID(ID)).getLevelData().getGDLevel().likes());
			JLabel downloads = createLabel("Downloads: " + RequestsTab.getRequest(Requests.getPosFromID(ID)).getLevelData().getGDLevel().downloads());
			JLabel length = createLabel("Length: " + RequestsTab.getRequest(Requests.getPosFromID(ID)).getLevelData().getGDLevel().length().toString());
			JLabel songID;
			JLabel songName;
			JLabel songAuthor;
			Optional<GDSong> song =  RequestsTab.getRequest(Requests.getPosFromID(ID)).getLevelData().getGDLevel().song();
		...
		setPreferredSize(new Dimension(RequestsTab.getLevelsPanel().getButtonWidth()-50, 120));
