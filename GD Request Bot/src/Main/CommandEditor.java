...
public class CommandEditor {
	...
	private static FancyTextArea commandNameText = new FancyTextArea(false, false);
	private static FancyTextArea commandResponse = new FancyTextArea(false, false);
	private static FancyTextArea soundFileLocation = new FancyTextArea(false, false);
	...
	private static JPanel basicPanel = new JPanel(null);
	...
	private static int sliderVal = 0;
					...
					try {
							...
							if (option.equals("YES")) {
								if(commandNameText.getText().trim().equalsIgnoreCase("")){
									new Thread(() ->{
										DialogBox.showDialogBox("$INVALID_NAME$", "$INVALID_NAME_INFO$", "", new String[]{"$OKAY$"});
	...
	public static void showEditor(String type) {
	...
	public static void saveStuff(String identifier, boolean state) {
		if (state) {
	...
	public static boolean getStuff(String identifier) {
		Path file = Paths.get(Defaults.saveDirectory + "\\GDBoard\\" + identifier + ".txt");
		if (Files.exists(file)) {
			Scanner sc = null;
			try {
	...
	public static void saveWhisper() {
