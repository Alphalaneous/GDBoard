...
public class CheerSettings {
	...
	public static JPanel createPanel() {
		...
		whisper.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				if (whisper.getSelectedState()) {
					Path file = Paths.get(Defaults.saveDirectory + "\\GDBoard\\cheerWhisper.txt");
					try {
					...
					try {
						if (Files.exists(file)) {
							...
							sc.close();
							if (exists) {
								Path temp = Paths.get(Defaults.saveDirectory + "\\GDBoard\\_tempCheerWhisper_");
								...
								out.close();
								Files.delete(file);
								Files.move(temp, temp.resolveSibling(Defaults.saveDirectory + "\\GDBoard\\cheerWhisper.txt"), StandardCopyOption.REPLACE_EXISTING);
							}
						}
					}
					catch (Exception f){
		...
		try {
			URI uri = Main.class.getResource("/Resources/cheers/").toURI();
			Path myPath;
			if (uri.getScheme().equals("jar")) {
				myPath = ServerChatBot.fileSystem.getPath("/Resources/cheers/");
			} else {
			...
			Path comPath = Paths.get(Defaults.saveDirectory + "/GDBoard/cheers/");
	...
	private static void showCommandPanel(String command, Path path){
		...
		CheerSettings.command = command;
		commandLabel.setText(command);
		commandLabel.setBounds(50,15,commandLabel.getPreferredSize().width+5, commandLabel.getPreferredSize().height + 5);
		StringBuilder function = new StringBuilder();
		try {
			Path comPath = Paths.get(Defaults.saveDirectory + "/GDBoard/cheers/" + command.split("\\\\")[command.split("\\\\").length-1] + ".js");
			if (Files.exists(comPath)) {
		...
		try {
			...
			if (Files.exists(file)) {
				Scanner sc = new Scanner(file);
				while (sc.hasNextLine()) {
					if (String.valueOf(CheerSettings.command).equals(sc.nextLine())) {
